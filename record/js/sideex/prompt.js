...
//play window methods
if (window == window.top) {
    window.addEventListener("message", sideexPromptMessage);
}
function functionremoveSideexPromptListener(){
    window.removeEventListener('message', sideexPromptMessage);
}
function sideexPromptMessage(event) {
    if ("source" in event && "data" in event && event.source == window && event.data &&
